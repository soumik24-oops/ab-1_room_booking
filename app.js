...
const USERS_KEY = 'ab1AuthorizedUsers';
...
function isIISEREmail(email){ return /^[^\s@]+@iiserb\.ac\.in$/i.test(email.trim()); }
function normalizeEmail(email){ return email.trim().toLowerCase(); }

/*
  Demo authorization source.
  In production, this file is replaced/synced from the private department-controlled
  repository. It is deliberately kept outside the UI logic so the authorization
  source can later be replaced by an API without rewriting the booking system.
*/
async function loadAuthorizedUsers(){
  const cached = localStorage.getItem(USERS_KEY);
  if(cached){ try { return JSON.parse(cached); } catch(_) {} }
  try {
    const res = await fetch('authorized-users.csv', {cache:'no-store'});
    if(!res.ok) throw new Error('Could not load authorization list');
    const text = await res.text();
    const lines = text.trim().split(/\r?\n/).slice(1);
    const users = lines.filter(Boolean).map(line => {
      const [email, roll_number, department, role, status] = line.split(',').map(v=>v.trim());
      return {email:normalizeEmail(email), roll_number, department, role, status};
    });
    localStorage.setItem(USERS_KEY, JSON.stringify(users));
    return users;
  } catch(err){
    authError.textContent='Authorization list could not be loaded. Please try again.';
    return [];
  }
}
...
async function authorize(email){
  ...
  if(!isIISEREmail(normalized)) return {ok:false,message:'Access denied. Please use your @iiserb.ac.in institutional email address.'};
  const user=authorizedUsers.find(u=>normalizeEmail(u.email)===normalized && u.status.toLowerCase()==='active');
  if(!user) return {ok:false,message:'Access denied. Your email is not on the authorized Physics/Mathematics user list.'};
  if(!['physics','mathematics'].includes(user.department.toLowerCase())) return {ok:false,message:'Access denied. This portal is restricted to Physics and Mathematics users.'};
  return {ok:true,user};
}

authForm.onsubmit = async e => {
  e.preventDefault();
  authError.textContent='Checking authorization…';
...
logout.onclick = () => { localStorage.removeItem(AUTH_KEY); showAuth(); };

(async()=>{
  authorizedUsers=await loadAuthorizedUsers();
  const saved=localStorage.getItem(AUTH_KEY);
  if(saved){
    try{
      const user=JSON.parse(saved);
      const result=await authorize(user.email);
      if(result.ok) showPortal(result.user); else showAuth();
    }catch(_){showAuth();}
  } else showAuth();
})();
